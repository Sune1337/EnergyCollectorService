...
public class EntsoeCollectorService : IEntsoeCollectorService
{
    ...
    public EntsoeCollectorService(ILogger<EntsoeCollectorService> logger, IEntsoeApiClient entsoeApiClient, IOptions<EntsoeApiOptions> options, IOptions<InfluxDbOptions> influxDbOptions)
    {
        _logger = logger;
  ...
  |> filter(fn: (r) => r[""_measurement""] == ""{_influxDbOptions.Value.Measurement}"")
  |> group()
  |> last(column: ""_time"")
", _influxDbOptions.Value.Organization, cancellationToken)
    ...
    private static bool TryParseIso8601Duration(string value, out TimeSpan timeSpan)
    {
        timeSpan = default;
    ...
    private async Task DownloadEnergyData(DateTime startDateTime, TimeSpan timeSpan, WriteApi influxWrite, CancellationToken cancellationToken)
    {
        ...
        var lastDates = new Dictionary<string, DateTime>();
        foreach (var pointsForDateTime in pointsPerDateTime)
        {
            ...
            foreach (var pointsForEnergyType in pointsPerEnergyType)
            {
                ...
                influxWrite.WritePoint(
                    PointData.Measurement(_influxDbOptions.Value.Measurement)
                ...
                lastDates.TryGetValue(pointsForEnergyType.Key, out var lastDate);
                if (minDateTime > lastDate)
                {
                    lastDates[pointsForEnergyType.Key] = minDateTime;
                }
            }
    ...
    private IEnumerable<T> EnumeratePeriodPoints<T>(TimeSeries? timeSerie, Func<string, string, DateTime, Point, T> func)
    {
        ...
        foreach (var period in timeSerie.Period)
        {
            ...
            if (!TryParseIso8601Duration(period.resolution, out var resolution))
            {
